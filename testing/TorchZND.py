...
class TorchZND:
	...
	def readZND(self):
		...
		for k in self.files.keys():
			...
			# the znd files I'm reading sometimes suffer from the first data line having different contents at the same time 0
			fklines = self.files[k].readlines()
			nklines = len(fklines)
			for i in range(0,nklines):
				fklines[i] = fklines[i].strip() # get rid of newline chars
				l = fklines[i].split()
				if l[0] == '1':
					l2 = fklines[i+1].split()
					if l2[0] == '1':
						l = l2
